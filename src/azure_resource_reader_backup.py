...
def handle_with_parse_mode(args) -> None:
    ...
    Args:
        args: 命令行参数
    """
    print(f"🔍 Azure Storage 资源读取器 (原始数据 + 解析数据)")
    print(f"📋 任务类型: {args.task_type_or_job_id}")
    print(f"📋 输入参数: {args.task_id_or_task_id}")
    ...
    for filename in files_to_process:
        ...
        if args.info_only:
            ...
            # 显示解析文件信息（只执行一次）
            if filename == files_to_process[0]:
                parse_reader = AzureResourceReader('collector0109')
        ...
        original_content = combined_content.get('original')
        ...
        if original_content is not None:
            ...
            if isinstance(original_content, str):
                print(f"📝 原始文件长度: {len(original_content)} 字符")
            else:
            ...
            success = _save_content_to_file(original_content, local_path)
            if success:
        ...
        if not parse_file_downloaded:
            if parse_content is not None:
                ...
                # 保存解析文件 - 如果检测到JSON内容，强制使用json格式
                parse_output_type = "json"  # 解析文件默认保存为json格式
                if isinstance(parse_content, str) and (parse_content.strip().startswith('{') or parse_content.strip().startswith('[')):
                    try:
                        json.loads(parse_content)  # 验证是否为有效JSON
                        print("🔍 检测到JSON内容，将保存为.json文件")
                    except json.JSONDecodeError:
                        pass  # 不是有效JSON，但仍使用json格式作为默认
                
                parse_filename = _generate_save_filename("parse_result", task_id, "json")  # 强制使用json格式
